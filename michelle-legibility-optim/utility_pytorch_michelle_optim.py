...
# #Hessian Matrix
# def hessian_homemade (x, the_func):
#   N = x.shape[0]
...
#     jacobian = Matrix(*element.shape, lambda i,j: fn[i,j].diff(element))

#     # partial_diff = function.diff(element)

#     return jacobian #partial_diff
...
# def hessian_showoff(fn, symbols_list, values_in):
#     """
#     Fonction principale.
#     """
#     function = fn
...
def compute_hessian(fn, values_list, values_in):
    # print("type f")
    # print(type(fn))
    print("function")
    print(fn)
    ...
    # return hessian_showoff(fn, values_list, values_in)

    # values = [np.asarray(values_in[0]), np.asarray(values_in[1])]
    # print(values)

    # return hessian_homemade(values_in, fn)

    # values_format = (lambda x: np.asarray(x))

    # values = values_format(values_in)
    # print(values)
    # print(type(values))
...
# Update call signs
def dynamics_jacobians(x_in, u_in, dt):
    # Example usage
    # X = Matrix([rho*cos(phi), rho*sin(phi), rho**2])
    # Y = Matrix([rho, phi])
    # X.jacobian(Y)
    x, u    = MatrixSymbol('x', 1, 2), MatrixSymbol('u', 1, 2)
    _x, _u  = MatrixSymbol('_x', 1, 2), MatrixSymbol('_u', 1, 2)
    # _x = Symbol('_x')
    # _u = Symbol('_u')
    ...
    print("x and u shape in dynamics_jacobians")
    print(x.shape)
    print(u.shape)
    ...
    rk4_A = Matrix([rk4_A]).jacobian([x])
    rk4_B = Matrix([rk4_B]).jacobian([u])


    # rk4_A.jacobian(x)
    # rk4_B.jacobian(u)

    print("rk4_A_jacobian")
    print(rk4_A)
    print("rk4_B_jacobian")
    print(rk4_B)

    A = rk4_A.subs([(x,x_in), (u, u_in)])
    B = rk4_B.subs([(x,x_in), (u, u_in)])
...
def term_cost_symbolic(x, xref):
    ...
    # returns a symbolic function of two variables
    return J, [x, xref]
...
def stage_cost_expansion(x_input, u_input, xref, uref, start, goal1, all_goals, nongoal_scale):
    ...
    # Notes on syntax in julia for gradient
    ...
    Jxx         = compute_hessian(dx, [x, u], [x_input, u_input])
...
def term_cost_expansion(x_input_t, xref_t):
    ...
    dx            = term_cost(x_input, xref_input)
    ...
    Jxx     = compute_hessian(dx_symbolic, dx_vars, [x_input, xref_input])
    Jxx     = Jxx.subs([(x,x_input), (xr, xref)])
    #hessian(dx_symbolic, (x, xr)) #x_input) # taken at 0
    Jx      = gradient(dx_symbolic, [x]) #x_input)
