...
def export_path_moments_confusion_for_each_goal(restaurant, best_paths, stamps, exp_settings):
	...
	for pkey in path_set:
		...
		solo_img = restaurant.get_img([], show_cones=False) #get_obs_img(obs_key, [resto.OBS_KEY_A, resto.OBS_KEY_E])
		all_obs_img = copy.copy(solo_img)

		for obs in ['a', 'b', 'c', 'd', 'e', None]:
			solo_img = restaurant.get_img([], show_cones=False)
			t_obs = obs
			...
			for i in range(len(path) - 1):
				...
				OVERTONE_FILL = (0,0,0) #(255,255,255)
...
def export_table_all_viewers(r, best_paths, exp_settings):
	...
	for key in best_paths:
		...
		for aud_key in obs_keys:
			...
			f_leg_value = legib.f_legibility(r, gkey, r.get_goals_all(), path, actual_audience, None, exp_settings)
			f_env_value, f_env_first, f_env_max = legib.f_env(r, gkey, r.get_goals_all(), path, actual_audience, None, exp_settings)
...
def analyze_all_paths(r, paths_for_analysis_dict, exp_settings):
	...
	for key in paths_for_analysis_dict:
		...
		for pi in range(len(paths)):
			...
			datum['path_length'] = legib.get_path_length(path)[1]
			datum['path_cost'] = legib.f_path_cost(path)
			data.append(datum)
			# datum = [goal_index] + []
...
def export_best_options():
	...
	if FLAG_EXPORT_MOCON and not FLAG_EXPORT_JUST_TEASER:
		stamps = {}
